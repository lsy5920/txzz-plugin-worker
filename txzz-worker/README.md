...
| --- | --- |
| Worker | `2.0.1` |
| 构建标识 | `txzz-worker-20260727-0851` |
| Wrangler | `4.110.0` |
| Node.js | `22.16.0` 及以上 |
| 数据库 | Supabase |
...
[2026-07-27 08:51] 【修复】升级 Worker 到 `2.0.1`（构建 `txzz-worker-20260727-0851`）：播放会话在返回前并行探测主备 HLS，支持递归解析主清单变体，并以候选线路的相对覆盖时长优先选择完整版，修复短预览主线可能压过较长备用线的问题；缓存命中也会重新核对线路完整度，不再复用旧错误决定。
[2026-07-27 08:51] 【测试】Node.js 自动化测试扩展到 21 项，新增 HLS 媒体时长/相对变体解析及短主线与长备用线选择回归；探测使用有限 Range，避免无扩展名 MP4 在诊断阶段被整段下载。
...
[2026-07-26 22:54] 【重构】升级 Worker 到 `1.4.0`（构建 `txzz-worker-20260726-2254`）：新增 `src/domain/playback.js` 和 `src/domain/diagnostics.js`，分别集中维护线路归一化、VIP 免购买保护和服务体检输出；`worker.js` 只导入并编排这些规则，原接口、鉴权、加密和 Supabase 契约保持兼容，11 项 Node.js 自动化测试全部通过。
[2026-07-26 23:20] 【修复】部署工作流启用 `pipefail`，避免 Wrangler 鉴权失败被日志管道错误标记为成功；新增发布后在线健康门禁，只有 `/v1/health` 返回当前 `BUILD_TAG` 且 `ready: true` 才视为部署完成。
