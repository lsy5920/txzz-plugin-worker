...
[2026-07-26 22:54] 【重构】升级 Worker 到 `1.4.0`（构建 `txzz-worker-20260726-2254`）：把无扩展名签名线路识别、主备线路归一化、VIP 免购买保护移入 `src/domain/playback.js`，把体检评分、检查项、建议和下一步动作移入 `src/domain/diagnostics.js`；路由、鉴权、AES 加密、Supabase 持久化与 API 响应契约保持不变，11 项自动化测试全部通过。
[2026-07-26 23:20] 【修复】修复 GitHub Actions 中 Wrangler 部署失败被 `tee` 吞掉退出码、工作流错误显示成功的问题；部署管道启用 `pipefail`，并在发布后强制核对线上 `/v1/health` 的构建标记与 `ready` 状态，Cloudflare 令牌失效、地址错误、旧构建或运行密钥缺失都会明确阻断发布。
[2026-07-27 01:00] 【重构】升级 Worker 到 `2.0.0`：新增 `/v2/playback/session`、五态购买幂等账本、缓存 schema v2 与 v1 30 天兼容适配器；直链检查、最低金币购买、扣费后对账和诊断门禁统一进入 PlaybackService。
[2026-07-27 10:15] 【修复】升级 Worker 到 `2.0.2`（构建 `txzz-worker-20260727-1015`）：递归收集 `lines[]` 等嵌套线路，无扩展名签名 HLS 收到 206 后无 Range 重取完整清单，并按每个视频的真实清单时长选择完整线路；23 项自动化测试全部通过。
